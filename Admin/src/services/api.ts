...
type Json = Record<string, any> | any[] | string | number | boolean | null | undefined;
...
class ApiService {
  ...
  async getBlogsAdmin({
    ...
    return this.request<{ items: Blog[]; page: number; limit: number; total: number; totalPages: number }>('/blogs', {
      qs: {
  ...
  async deleteBlog(id: string) {
    return this.request<{ message: string; post: Blog }>(`/blogs/${id}`, { method: 'DELETE' });
  }

  async restoreBlog(id: string) {
    return this.request<{ message: string; post: Blog }>(`/blogs/${id}/restore`, { method: 'POST' });
  }
