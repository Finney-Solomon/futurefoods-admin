...
class ApiService {
  ...
  async cancelOrder(orderId: string) {
    const res = await this.request<any>(`/orders/${orderId}/cancel`, {
      method: "PATCH",
    });
    return res?.order || res;
  }

  async markOrderShipped(orderId: string, data: Json) {
    const res = await this.request<any>(`/orders/${orderId}/ship`, {
      method: "PATCH",
      body: JSON.stringify(data),
    });
    return res?.order || res;
  ...
  async markOrderDelivered(orderId: string) {
    const res = await this.request<any>(`/orders/${orderId}/deliver`, {
      method: "PATCH",
    });
    return res?.order || res;
  }

  async confirmOrderPayment(orderId: string, paymentIntentId: string) {
    const res = await this.request<any>(`/orders/${orderId}/confirm-payment`, {
      method: "POST",
  ...
  async adminCancelOrder(orderId: string) {
    const res = await this.request<any>(`/orders/${orderId}/admin-cancel`, {
      method: "PATCH",
    });
    return res?.order || res;
  }
  ...
  async restoreBlog(id: string) {
    return this.request<{ message: string; post: Blog }>(
      `/blogs/${id}/restore`,
      {
        method: "POST",
      },
  ...
  async updateReviewVisibility(id: string, isVisible: boolean) {
    return this.request<any>(`/reviews/${id}/visibility`, {
      method: "PATCH",
